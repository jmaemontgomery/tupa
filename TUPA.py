...
def elec_field_per_traj_para(frame_index, u, elecfield_selection, probe_selection, config):
    ...
    time = config.dt * (int(frame_index) + 1)
    ...
    return time, totalEfmag, totalEf, refposition, lineprobe, lineEfield
...
# Create variable to iterate through function
run_per_frame = partial(elec_field_per_traj_para, u=u, elecfield_selection=elecfield_selection, probe_selection=probe_selection, config=config)

# Create varible for all trajectory frames
frame_values = np.arange(u.trajectory.n_frames)

# Parallelize jobs by creating a pool to iterate over frames (frame_values) using the partial function (run_per_frame)
with Pool(n_jobs) as worker_pool:
        newresults = worker_pool.map(run_per_frame, frame_values)

# Loop over the results and write them to the output files
for time, totalEfmag, totalEf, refposition, lineprobe, lineEfield in newresults:
    outprobe.write(lineprobe)
    out.write(lineEfield)
    # ... (write other results to corresponding output files)

# Close the output files
outprobe.close()
out.close()
# ... (close other output files)

print(results.efield_timeseries)

# result post-processing commented out due to different handling needed due to parallelization (JMM | 8/2023)
